            ...
            <div className="flex-1 overflow-y-auto space-y-4 text-xs pr-1">
              ...
              {activeTabWorkspace === 'absensi' && (() => {
                const attendanceUrl = typeof window !== 'undefined' 
                  ? `${window.location.origin}/absensi/${detailProposal.id}` 
                  : `/absensi/${detailProposal.id}`;
                
                const currentRecords = attendanceRecords.filter(r => 
                  r.activityId === detailProposal.id || ensureUUID(r.activityId) === ensureUUID(detailProposal.id)
                );
                const verifiedCount = currentRecords.filter(r => r.status === 'verified').length;
                const unverifiedCount = currentRecords.filter(r => r.status === 'unverified').length;

                return (
                  <div className="space-y-5">
                    {/* Top Box: QR Code & Link Absensi */}
                    <div className="bg-slate-50 p-4 sm:p-5 rounded-2xl border border-slate-200 space-y-3">
                      <div className="font-bold text-slate-900 flex items-center justify-between">
                        <span className="text-xs sm:text-sm">Presensi Online & QR Code Absensi:</span>
                      ...
                      <div className="bg-white p-4 sm:p-5 rounded-xl border border-slate-200 flex flex-col sm:flex-row items-center gap-4 sm:gap-5">
                        <div className="w-28 h-28 bg-white border border-slate-200 p-1.5 rounded-xl flex items-center justify-center shrink-0 shadow-sm relative group">
                          <img 
                          ...
                          />
                          <div className="absolute inset-0 flex items-center justify-center text-dwp-burgundy pointer-events-none -z-10">
                            <QrCode className="w-14 h-14 text-dwp-gold" />
                          </div>
                        </div>

                        <div className="space-y-2 flex-1 w-full text-center sm:text-left">
                          <div className="font-bold text-slate-900 text-xs sm:text-sm">Link Absensi Online Kegiatan:</div>
                          
                          <div className="bg-slate-100 p-2.5 rounded-lg text-[11px] font-mono text-slate-800 flex flex-col sm:flex-row sm:items-center justify-between gap-2 border border-slate-200">
                            <div className="flex items-center gap-2 truncate">
                              <Link className="w-3.5 h-3.5 text-dwp-burgundy shrink-0" />
                              <span className="truncate">{attendanceUrl}</span>
                            </div>
                            <div className="flex items-center gap-2 shrink-0 self-end sm:self-auto">
                              <button
                                type="button"
                                onClick={() => copyToClipboard(attendanceUrl)}
                                className="bg-white hover:bg-slate-50 border border-slate-300 text-slate-700 font-bold px-2.5 py-1 rounded-md text-[10px] shadow-sm flex items-center gap-1 transition-all"
                              >
                                {copiedLink ? (
                                  <>
                              ...
                              <a
                                ...
                                className="bg-dwp-burgundy hover:bg-dwp-darkBurgundy text-white font-bold px-2.5 py-1 rounded-md text-[10px] shadow-sm flex items-center gap-1 transition-all"
                              >
                                <ExternalLink className="w-3 h-3 text-dwp-gold" />
                                <span>Buka Form</span>
                              </a>
                            </div>
                          </div>
                    ...
                    {/* Bottom Box: Daftar Hadir Peserta */}
                    <div className="bg-white p-4 sm:p-5 rounded-2xl border border-slate-200 shadow-sm space-y-4">
                      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-slate-100 pb-3">
                        <div className="space-y-1">
                          ...
                          <p className="text-[11px] text-slate-500">
                            Data kehadiran peserta yang mengisi via tautan atau QR Code di atas. Pengurus dapat memverifikasi presensi peserta.
                          </p>
                        </div>
                        ...
                        <button
                          ...
                          className="bg-slate-100 hover:bg-slate-200 text-slate-700 font-semibold px-3 py-1.5 rounded-xl text-xs flex items-center gap-1.5 self-start sm:self-auto transition-all"
                        >
                          <RotateCcw className={`w-3.5 h-3.5 text-dwp-burgundy ${isRefreshingAttendance ? 'animate-spin' : ''}`} />
                          <span>{isRefreshingAttendance ? 'Memuat...' : 'Segarkan Data'}</span>
                        </button>
                      </div>
                      ...
                      {currentRecords.length === 0 ? (
                        <div className="text-center py-8 text-slate-400 text-xs space-y-2">
                          ...
                          <p className="font-medium text-slate-600">Belum ada peserta yang mengisi daftar hadir untuk kegiatan ini.</p>
                          <p className="text-[10px] text-slate-400">
                            Bagikan link atau tampilkan QR Code di atas kepada peserta kegiatan.
                          </p>
                        </div>
                      ) : (
                          ...
                          <table className="w-full text-left text-xs border-collapse">
                            <thead>
                              <tr className="bg-slate-50 text-slate-700 font-bold border-b border-slate-200 text-[11px]">
                                ...
                                <th className="p-2.5">Jabatan / Instansi</th>
                                <th className="p-2.5 text-center">Waktu</th>
                                <th className="p-2.5 text-center w-24">Tanda Tangan</th>
                                <th className="p-2.5 text-center">Status & Aksi</th>
                              </tr>
                            </thead>
                              ...
                              {currentRecords.map((rec, idx) => (
                                <tr key={rec.id} className="hover:bg-slate-50 transition-colors">
                                  ...
                                  <td className="p-2.5">
                                    ...
                                    {rec.nip && <div className="text-[10px] text-slate-500 font-mono">NIP. {rec.nip}</div>}
                                    {rec.phone && <div className="text-[10px] text-slate-400">HP: {rec.phone}</div>}
                                  </td>
                                  <td className="p-2.5 text-slate-600">{rec.jabatan || 'Peserta'}</td>
                                  <td className="p-2.5 text-center font-mono text-[11px] text-slate-600 whitespace-nowrap">
                                    {rec.checkInTime}
                                  </td>
                                  <td className="p-2.5 text-center">
                                    {rec.signatureUrl ? (
                                      <div className="bg-white border border-slate-200 rounded p-1 inline-block shadow-sm">
                                        <img 
                                          src={rec.signatureUrl} 
                                          alt="TTD" 
                                          className="h-7 max-w-[80px] object-contain mx-auto"
                                        />
                                      </div>
                                    ) : (
                                  ...
                                  <td className="p-2.5 text-center">
                                    {rec.status === 'verified' ? (
                                      <div className="space-y-0.5">
                                        ...
                                        {rec.verifiedBy && (
                                          <div className="text-[9px] text-slate-400">oleh: {rec.verifiedBy}</div>
                                        )}
                                      </div>
                                    ) : (
                                      <div className="flex flex-col sm:flex-row items-center justify-center gap-1.5">
                                        ...
                                        <button
                                          ...
                                          className="bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-[10px] px-2.5 py-1 rounded-lg shadow-sm flex items-center gap-1 transition-all"
                                          title="Verifikasi kehadiran peserta ini"
                                        >
